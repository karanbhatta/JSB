...
class Model_sales extends CI_Model
{
 ...
 public function getCustomer(){
  ...
  public function getCustomerBySale($id){
	...
	{
		if($id) {
			$sql = "SELECT * FROM sales WHERE vehicle_no = ?";
			$query = $this->db->query($sql, array($id));
			return $query->result_array();
		}
...
$c=$cd['customer_name'];
	...
	// total sales
public function countTotalSales($sd=null, $ed=null){

if($sd) {
			$sql = "SELECT sum(net_amount) as net_amount  FROM sales WHERE date_time  BETWEEN ? AND ? ";
			$query = $this->db->query($sql, array($sd, $ed));
			return $query->row_array();
		}

		$sql = "SELECT sum(net_amount) as net_amount FROM sales";
		$query = $this->db->query($sql);
		return $query->row_array();
