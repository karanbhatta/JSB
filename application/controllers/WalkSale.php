...
class WalkSale extends Admin_Controller 
{
...
if($language=='np')	{
 ...
 $vehicle_no=$this->input->post('vehicle_no');
		...
		$this->data['language'] = $language == '' ? 'en' : $language;
		$this->data['page_title'] = 'Manage Sales Entry';
		$this->render_template('walk_sale/index', $this->data);		
	}
	...
	*/
	...
	// echo $vehicle_no;
		...
		$data = $this->model_walk_sales->getOrdersData();

		foreach ($data as $key => $value) {

			$count_total_item = $this->model_walk_sales->countOrderItem($value['id']);
			...
			if(in_array('viewOrder', $this->permission)) {
				$buttons .= '<a target="__blank" href="'.base_url('WalkSale/printDiv/'.$value['id']).'" class="btn btn-default"><i class="fa fa-print"></i></a>';
			}

			if(in_array('updateOrder', $this->permission)) {
				$buttons .= ' <a href="'.base_url('WalkSale/update/'.$value['id']).'" class="btn btn-default"><i class="fa fa-pencil"></i></a>';
			}
	...
	*/
	public function create()
	{
        ...
        if ($this->form_validation->run() == TRUE) {        	
        	...
        	$order_id = $this->model_walk_sales->create();
        	
        	if($order_id) {
        		$this->session->set_flashdata('success', 'Successfully created');
        		redirect('WalkSale/update/'.$order_id, 'refresh');
        	}
        	else {
        		$this->session->set_flashdata('errors', 'Error occurred!!');
        		redirect('WalkSale/create/', 'refresh');
        	}
        } 
        else {
            // false case
        	...
        	$this->data['customer'] = $this->model_walk_sales->getCustomer();      	
		...
		$this->data['language'] = $language == '' ? 'en' : $language;
            ...
            $this->render_template('walk_sale/create', $this->data);
        }	
	}
	...
	*/
	public function update($id)
	{
        ...
        if ($this->form_validation->run() == TRUE) {        	
        	...
        	$update = $this->model_walk_sales->update($id);
        	
        	if($update == true) {
        		$this->session->set_flashdata('success', 'Successfully updated');
        		redirect('WalkSale/index/'.$id, 'refresh');
        	}
        	else {
        		$this->session->set_flashdata('errors', 'Error occurred!!');
        		redirect('WalkSale/update/'.$id, 'refresh');
        	}
        }
        else {
            // false case
        	...
        	$result = array();
        	$orders_data = $this->model_walk_sales->getSalesData($id);
 
    		$result['order'] = $orders_data;
    		
    		$orders_item = $this->model_walk_sales->getSalesItemData($orders_data['id']);
		...
		$this->data['language'] = $language == '' ? 'en' : $language;
            ...
            $this->render_template('walk_sale/edit', $this->data);
        }
	}
...
if($language=='np')	{
	...
	*/
	public function printDiv($id) 
	{
		...
		if($id) {
			$order_data = $this->model_walk_sales->getOrdersDataDetail($id);
			$orders_items = $this->model_walk_sales->getSalesItemData($id);
			      ...
			      <div class="col-xs-6 pull pull-right">
			        ...
			        <div class="table-responsive">
			          <table class="table">
			            ...
			            <tr>
			              <th>Paid Amount:</th>
			              <td>'.$order_data['paid_amount'].'</td>
			            </tr>
			            <tr>
			              <th>Due Amount:</th>
			              <td>'.$order_data['due_amount'].'</td>
			            </tr>
			            <tr>
			              <th>Paid Status:</th>
			              <td>'.$paid_status.'</td>
